# # -------------------------
# # Generative model for POMCP
# # -------------------------
# function POMDPs.gen(p::PlaneLandingPOMDP, s::State, a::Action, sp::State)
#     next_state, reward, done = step(s, a, p.sim_config, p.run_config)
#     obs = observation(next_state, p.sim_config, p.run_config)
#     return (sp=next_state, o=obs, r=reward, terminal=done)  
# end
...
# # include(joinpath(@__DIR__, "simulation/dynamics.jl"))
...
# #     sim_action = Action(action.dThrottle, round(Int, action.dPitch))
...
# # # pitch_limits = [-5.0, 5.0]
# # # action_config = PlanePOMDP.ActionBoundsConfig(throttle_limits, pitch_limits)
